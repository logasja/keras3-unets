...
class patch_extract(Layer):
    ...
    def call(self, images):
        ...
        images = ops.cast(images, dtype=K.floatx())
        ...
        patches = ops.reshape(patches, (batch_size, patch_num * patch_num, patch_dim))
        # patches.shape = (num_sample, patch_num*patch_num, patch_size*channel)
...
class patch_expanding(Layer):
    ...
    def call(self, x):
        ...
        if self.return_vector:
            ...
            x = ops.reshape(
                x, (-1, L * self.upsample_rate * self.upsample_rate, C // 2)
            )
...
def drop_path_(inputs, drop_prob, is_training):
    ...
    random_tensor = keep_prob + random.uniform(shape, dtype=inputs.dtype)
    path_mask = ops.floor(random_tensor)
    output = ops.divide(inputs, keep_prob) * path_mask
    return output
...
class Mlp(Layer):
    def __init__(self, filter_num, drop=0.0, name="mlp", **kwargs):
...
class WindowAttention(Layer):
    ...
    def call(self, x, mask=None):
        ...
        k = ops.transpose(k, perm=(0, 1, 3, 2))
        attn = q @ k
        ...
        relative_position_index_flat = ops.cast(relative_position_index_flat, "int32")
        ...
        if mask is not None:
            ...
            mask_float = ops.cast(
                ops.expand_dims(ops.expand_dims(mask, axis=1), axis=0), dtype=attn_dtype
            )
